...
NoeudArbre* rechercherRacine(NoeudArbre* racine, const char* cle) {
    if (!cle || racine == NULL) return NULL;
...
// -----------------------------------------------------------------------------
// Affichage trié alphabétiquement
// -----------------------------------------------------------------------------
...
void afficherRacinesCallback(NoeudArbre *racine, AppendFunc append) {
    ...
    // ✅ CORRECTION : buffer agrandi de 100 → 256 pour éviter la troncature
    char footer[256];
...
void afficherDerivesRacine(NoeudArbre* noeud, AppendFunc append) {
    ...
    if (!noeud || noeud->data.nbDerives == 0) {
        // ✅ CORRECTION : buffer agrandi de 200 → 300
        char msg[300];
        if (noeud) {
    ...
    // ✅ CORRECTION : on utilise plusieurs appels append() au lieu d'un seul
    // gros snprintf() pour éviter la troncature
    char line[256];

    append("\n╔═══════════════════════════════════════════════════╗\n");

    snprintf(line, sizeof(line), "║   📖 Dérivés de : %s\n", noeud->data.racine);
    append(line);
...
void afficherStatistiquesArbre(NoeudArbre* racine, AppendFunc append) {
    ...
    // ✅ CORRECTION : on découpe en plusieurs appels append() au lieu d'un
    // unique snprintf() dans un buffer de 400 octets (insuffisant)
    append("\n╔═══════════════════════════════════════════════════╗\n");
    append("║          📊 STATISTIQUES DE L'ARBRE              ║\n");
    append("╚═══════════════════════════════════════════════════╝\n\n");

    char line[256];

    snprintf(line, sizeof(line), "  🌳 Nombre de racines     : %d\n", nbRacines);
    append(line);

    snprintf(line, sizeof(line), "  📖 Nombre de dérivés     : %d\n", nbDerives);
    append(line);

    snprintf(line, sizeof(line), "  📏 Hauteur de l'arbre    : %d\n", hauteurMax);
    append(line);

    snprintf(line, sizeof(line), "  📈 Moyenne dérivés/racine: %.2f\n",
             nbRacines > 0 ? (float)nbDerives / nbRacines : 0.0f);
    append(line);

    snprintf(line, sizeof(line), "  ⚖️  Facteur d'équilibre   : %d\n\n",
             equilibre(racine));
    append(line);
}
